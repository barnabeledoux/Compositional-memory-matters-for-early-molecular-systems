...
class GillespieA:
    def __init__(self, T, K, repet, N, xini, lamdaini, alpha, gamma, mu, nu, mmutmax, ymutmax, dilu, ratiolen = 4, kappa = [], tau = [], printinfo=False, mutantfirst=False, history=False, begin=[]):
    ...
    def step(self, comp, carryingcap = False): #Maturation of one of the different compartments (Gillespie simulation of the parallel evolutions in each compartment)
        ...
        if not(carryingcap): #If the compartment has not reached the carrying capacity yet
            for i in range(self.mu.size):
                for j in range(self.mu.size):
                    ...
                    ##print('i = ', i, 'j = ', j,'S = ', S, 'marr = ', marr, ' alpha = ', self.alpha, ' yarr = ', yarr)
            for i in range(self.mu.size):
        ...
        while R < x2 * S:
            if not(carryingcap):
                ##print('i = ', i,'j = ', j,'imut = ', imut,'jmut = ', jmut,'R = ', R)
        ...
        for p, comp in enumerate(self.comp):
            ...
            if sum(comp[0])>0: #If the compartment is not empty
                ...
                if (((sum(comp[0])+sum(comp[1]/self.ratiolen))<self.K+nini) and (tint<self.T)): #If the compartment has not reached its carrying capacity yet and the maturation time has not been exceeded
                    ...
                    comp[-1] += 1
                    ##print('tint_bef_addit_mut = ', comp[2][1])
                    res=self.step(comp, carryingcap) #res=[i,j,imut,jmut,mut]
                    i,j,imut,jmut,mut = res[0], res[1], res[2], res[3], res[4]
                    ##Update the populations##
                    #We know that a mutation has been picked
                    if comp[2][1]<self.T:
                        ##print('tint_af = ', comp[2][1], ' < ', self.T)
                        if (imut >= self.mu.size): #If a parasite has muted
                            if ((comp[4][1]==0 or comp[5][1]==0) and comp[4][0]>0 and comp[5][0]>0):
                                self.ymutcount[0] += 1
                            else:
    ...
    def pooling(self): #After the maturation step, perform the pooling again by emptying the compartments in a common pool
        ...
        #if self.printinfo:
            #print('number replicases = ', self.mmutant, 'number parasites = ', self.ymutant)
        ##print('number replicases = ', self.mmutant, 'number parasites = ', self.ymutant, ' N = ', self.N, 'dilu = ', self.dilu, ' lamda = ', (sum(self.mmutant)+sum(self.ymutant))/self.N/self.dilu)
    ...
    def evolv(self, lamcond=False, xcond=False, carryingcaplim = True):
        ...
        for k in range(self.repet): #Repeat the process repet times
            ...
            ##if self.printinfo:
            ##print('repet = ', k, [self.mmutant,self.ymutant])
            if (sum(self.ymutant)<1.): #If there still are replicases
                self.compart()
                #if self.printinfo:
                    #print('mmutant = ', self.mmutant, 'ymutant = ', self.ymutant)
                ##print('after compart', self.comp)
                t=0
                condition_end=False
                while not(condition_end): #while t<self.T:
                    ntot = 0
                    ##print('compartments = ', self.comp)
                    self.allstep()
                    ##print('tmin = ', t)
                    condition_end=True
                    tlist=[]
                    for i,comp in enumerate(self.comp):
                        ...
                        if carryingcaplim : #If the evolution is limited by the carrying capacity
                            if ((sum(comp[0])+sum(comp[1])/self.ratiolen)<self.K+nini and sum(comp[0])>0 and tint<self.T):
                                ...
                                condition_end = False
                                #if (not(condition_end) and former_cond):
                                #    #print('comp_pb = ', comp, condition_end, (sum(comp[0])+sum(comp[1])/self.ratiolen)<self.K+nini, sum(comp[0])>0)
                            ##print(r'full cells = ', ntot/self.N)
                ...
                for comp in self.comp: #print anomalies
                    ...
                    if ((mini[0]>0) and (mini[1]==0) and (yini[0]==0)):
                        if comp[0][0]<self.K*0.8 :
                            ##print('proba (x_0) \sim ', comp[0][0] * self.alpha[0,0] * comp[0][0] * self.T)
                            ##print('x_0', comp)
                            x0ano+=1
                        ...
                        if comp[1][1]/self.ratiolen<self.K*0.7 :
                            ##print('y_1', comp)
                            z1ano+=1
            #nano = x0ano + x1ano + z0ano + z1ano
            #eff = 1. - nano / ntot
            #x0ano, x1ano, z0ano, z1ano = x0ano/nano, x1ano/nano, z0ano/nano, z1ano/nano
            ##print('eff = ', eff, 'x0ano = ', x0ano, 'x1ano = ', x1ano, 'z0ano = ', z0ano, 'z1ano = ', z1ano)
                ##print('comp_after = ', self.comp)
                ##print(r'tmax = ', max(tlist), r'log(K)/alpha00 = ', np.log(self.K)/self.alpha[0][0])
            ##print('repet = ', k, 'mmutfav = ', self.mmutcount[0]/self.N, 'mmutunfav = ', self.mmutcount[1]/self.N, 'ymutfav = ', self.ymutcount[0]/self.N, 'ymutunfav = ', self.ymutcount[1]/self.N)
            self.pooling()
            repart.append([self.mmutant,self.ymutant])
            lam.append(self.lamda)
            x.append(sum(self.mmutant))
            #if self.history:
            #    for i, hist in enumerate(comp_hist):
            #        if len(hist)>0:
            #            cond1 = (0==1) #((hist[-1][0][1]+hist[-1][1][1]>self.K*0.8)
            #            if (cond1 and (sum(hist[-1][0])>0)):
            #                #print(hist)
            #                #print(self.comp[i])
        if lamcond:
            if xcond:
...
class theory_compartA:
    def __init__(self, T, K, d, x0ini, x1ini, z0ini, lamdaini, repet, alpha, gamma, mu, nu, ratiolen = 4, ncomp = 1E3, th= 1E-9, minrep = False):
        self.T = T
        self.K = K
        self.d = d
        self.ratiolen = ratiolen
        ...
        self.alpha = alpha
        self.fastmut = False
        #if (self.mu[0] > 1.E1*self.alpha[0][0]) and (self.nu[0] > 1.E1*self.gamma[0][0]):
        #    self.fastmut = True
        self.smartmut = False
        if (self.gamma[0][1] > 0.):
    ...
    def x0star(self):
        ...
        ##print('m0 nomut ', m0starnomut, 'm0 mut ', m0starmutfav, '  ', m0starmutunfav)
        #if m0starnomut + m0starmutfav + m0starmutunfav < 0:
        #    self.fastmut = True
    ...
    def x1star(self):
        ...
        #if (m1starnomut + m1starmutfav + m1starmutunfav) > nstar:
        #    self.fastmut = True
        if x0 == 0. or self.mu[0]==0.: #Case when there is no mutations of replicases
            m1starmutfav = 0.
            m1starmutunfav = 0.
    ...
    def z0star(self):
        ...
        #if (y0starnomut+y0starmutfav+y0starmutunfav)<0.:
        #    self.fastmut = True
    ...
    def z1star(self):
        ...
        #if (y1starnomut + y1starmutfav + y1starmutunfav)>nstar:
        #    self.fastmut = True
        if z0 == 0. or self.nu[0]==0.: #Case when there is no mutations of replicases
            y1starmutfav = 0.
            y1starmutunfav = 0.
    ...
    def evol(self, noise = False, lambdaarr=False):
        ...
        for k in range(self.repet):
            ...
            ##print(r'Rough estim mut = ', self.lamda*(self.x0+self.x1)*self.mu[0]/self.alpha[0][0])
            ##print('repet = ', k, 'mmutfav = ', self.mmutcount[0], 'mmutunfav = ', self.mmutcount[1], 'ymutfav = ', self.ymutcount[0], 'ymutunfav = ', self.ymutcount[1])
        if noise:
...
class theory_compartA_stir:
    ...
    def update(self, monit_Xbef = False):
        ...
        for i, compart in enumerate(self.comp):
            m0i, m1i, y0i, y1i, ni = compart[0], compart[1], compart[2], compart[3], sum(compart)
            #x0i, x1i, z0i, z1i = (1-s)*m0i/(ni + (ni==0)) + s*x0, (1-s)*m1i/(ni + (ni==0)) + s*x1, (1-s)*y0i/(ni + (ni==0)) + s*z0, (1-s)*y1i/(ni + (ni==0)) + s*z1
            ...
            lami += (lami==0.)
            x0i, x1i, z0i, z1i = m0i/lami, m1i/lami, y0i/lami, y1i/lami #fraction after maturation and before stirring
            if monit_Xbef:
                xbefore = x0i + x1i
        ...
        ##print(self.x0, self.x1, self.z0, self.z1)
...
class GillespieB:
    ...
    def pooling(self): #After the maturation step, perform the pooling again by emptying the compartments in a common pool
        ...
        self.mmutant = [self.mmutant[i]/ntot for i in range(comp[0].size)] #Creates the fractions of each mutant of replicases
        self.ymutant = [self.ymutant[i]/ntot for i in range(comp[1].size)] #Creates the fractions of each mutant of parasites
        ##print(r'Gil m_0=',self.lamda*self.mmutant[0]*self.dilu,r'm_1=',self.lamda*self.mmutant[1]*self.dilu,r'y_0=',self.lamda*self.ymutant[0]*self.dilu,r'y_1=',self.lamda*self.ymutant[1]*self.dilu)
    ...
    def evolv(self, fraction = False, lamcond=False, xcond=False, carryingcaplim = True):
        ...
        for k in range(self.repet): #Repeat the process repet times
            ...
            if (sum(self.ymutant)<1.): #If there still are replicases
                ...
                while not(condition_end): #while t<self.T:
                    ntot = 0
                    ##print('compartments = ', [(i,list([float(x) for x in comp[0]]), list([float(y) for y in comp[1]])) for i,comp in enumerate(self.comp)])
                    self.allstep()
                    ##print('tmin = ', t)
                    condition_end=True
                    tlist=[]
                    for i,comp in enumerate(self.comp):
                        ...
                        if carryingcaplim : #If the evolution is limited by the carrying capacity
                            if ((sum(comp[0])+sum(comp[1])/self.ratiolen)<self.K+nini and sum(comp[0])>0 and tint<self.T):
                                ...
                                condition_end = False
                                #if (not(condition_end) and former_cond):
                                #    #print('comp_pb = ', comp, condition_end, (sum(comp[0])+sum(comp[1]))<self.K+nini, sum(comp[0])>0)
                            ##print(r'full cells = ', ntot/self.N)
                        else : #If the evolution is a limited by a time of maturation
                            if ((tint<self.T) and sum(comp[0])>0 and (tint != tintm1)):
                                condition_end = False
    ...
    def update(self, monit_Xbef=False):
        ...
        for i, compart in enumerate(self.comp):
            ...
            if (m1i < 1) and (y0i < 1) and (m0i > 0):
                ...
                compart[3] += K*self.ratiolen
            if self.history:
                hist_compbycomp_step.append([compart[0], compart[1], compart[2], compart[3]]) #append([m0i/(ni+(ni==0)), m1i/(ni+(ni==0)), y0i/(ni+(ni==0)), y1i/(ni+(ni==0))])
            #mutations
...
class theory_compart:
    ...
    def evol(self):
        ...
        for k in range(self.repet):
            ...
            ##print(r'sum = ', self.x0+self.x1+self.z0+self.z1)
            ##print(r'x_0=',self.x0,r'x_1=',self.x1,r'z_0=',self.z0,r'z_1=',self.z1)
            ##print(r'theo m_0=',self.m0,r'm_1=',self.m1,r'y_0=',self.y0,r'y_1=',self.y1)
            hist_evol.append([self.mmutant,self.ymutant])
            if self.x0+self.x1+self.z0+self.z1>1.1:
                ##print(self.d, self.K)
                hist_evol.append([mcop,ycop])
...
class theory_compart_stir_betedge:
    ...
    def update(self):
        ...
        if self.history:
            self.hist_compbycomp.append(hist_compbycomp_step)
        Sn = Sm0 + Sm1 + Sy0 + Sy1
        ##print('m0 stir ', Sm0/ncomp)
        ##print('m1 stir ', Sm1/ncomp)
        ##print('y0 stir ', Sy0/ncomp)
        ##print('y1 stir ', Sy1/ncomp)
        ##print('n/d stir ', Sn/d/ncomp)
        x0, x1, z0, z1 = Sm0/(Sn + (Sn==0)), Sm1/(Sn + (Sn==0)), Sy0/(Sn + (Sn==0)), Sy1/(Sn + (Sn==0))
        self.mmutantnum, self.ymutantnum = np.array([Sm0, Sm1]), np.array([Sy0, Sy1])
        ## Dilute ##
        Ndil = ncomp//d
